// src/infrastructure/security/two-factor.service.js
...
/**
 * Serviço para autenticação em dois fatores (2FA)
 */
class TwoFactorService {
  ...
  generateSecret(userEmail, appName = 'Mercearia Digital') {
    try {
      ...
      throw new InternalServerError('Erro ao gerar segredo para autenticação de dois fatores');
    }
  }
  ...
  verifyToken(secret, token, window = 1) {
    try {
      ...
      throw new InternalServerError('Erro ao verificar token de autenticação');
    }
  }
  ...
  async generateQRCode(otpauthUrl) {
    try {
      ...
      throw new InternalServerError('Erro ao gerar QR Code para configuração 2FA');
    }
  }
  ...
  generateBackupToken(secret) {
    try {
      ...
      throw new InternalServerError('Erro ao gerar token de recuperação');
    }
  }
